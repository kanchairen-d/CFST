#!/usr/bin/env python3
"""
Cloudflare IP 优选工具 (TCP筛选 + IP可用性二次筛选 + HTTP检测 + curl带宽测速 + 微信通知（WxPusher/PushPlus）)
...
def send_wxpusher_notification(content, summary):
    send_notification(content, summary)
...
def calibrate_regions(nodes, token_file, cache_file):
    ...
    # Read tokens from config array, fall back to file
    token_list = cfg.get("IP_CALIBRATION_TOKENS", [])
    if not token_list:
        token_list = load_tokens(token_file)

    if not token_list:
        print("Token 为空（未在设置页填写，也未找到文件），IP 地区校准跳过。")
...
def check_availability(node_str):
    ...
    # AVAILABILITY_CHECK_API 可以是字符串（向后兼容）或数组
    api_urls = AVAILABILITY_CHECK_API if isinstance(AVAILABILITY_CHECK_API, list) else [AVAILABILITY_CHECK_API]
...
def sync_to_github():
    ...
    if not all([token, username, repo]):
        print("GitHub 配置不完整（Token/用户名/仓库），跳过同步。")
        return
    ...
    for attempt in range(1, GITHUB_SYNC_MAX_RETRIES + 1):
        ...
        try:
            cmds = [
                ["git", "add", "ip.txt"],
                ["git", "-c", "user.name=cfst", "-c", "user.email=cfst@local",
                 "commit", "--allow-empty", "-m", f"Auto sync {time.strftime('%Y-%m-%d %H:%M')}"],
                ["git", "push", remote_url, f"HEAD:{branch}"],
            ]
            for cmd in cmds:
                r = subprocess.run(cmd, capture_output=True, text=True,
                                   timeout=GIT_SYNC_PROCESS_TIMEOUT, cwd=script_dir)
                if r.returncode != 0:
                    err = r.stderr.strip()
    ...
    send_wxpusher_notification(
        content=f"GitHub 推送失败，已重试 {GITHUB_SYNC_MAX_RETRIES} 次，请检查网络或 GitHub 仓库状态。",
        summary="GitHub 推送失败"
    )
...
def main():
    ...
    if cfg.get("GITHUB_SYNC_ENABLED", False):
        sync_to_github()

    # 发送测速完成通知
    node_count = len(final_selected) if final_selected else 0
    best_bw = max(speed_map.values()) if speed_map else 0
    msg_lines = [f"✅ 测速完成"]
    if node_count:
        msg_lines.append(f"优选节点：{node_count} 个")
    if best_bw:
        msg_lines.append(f"最高带宽：{best_bw:.2f} Mbps")
    # 同步通知由后台自动触发
    send_wxpusher_notification(content="\n".join(msg_lines), summary="测速完成")
