...
def _auto_sync_github():
    ...
    if not token or not username or not repo:
        _send_notification("GitHub 同步", "❌ GitHub 配置不完整，跳过自动同步")
        return
    conn = get_db()
    row = conn.execute(
        "SELECT results_json FROM runs WHERE status='completed' ORDER BY id DESC LIMIT 1"
    ...
    conn.close()
    if not row:
        _send_notification("GitHub 同步", "❌ 没有已完成的历史记录，跳过自动同步")
        return
    try:
        results = json.loads(row["results_json"])
        nodes = results.get("nodes", [])
        ...
        if not ip_lines:
            _send_notification("GitHub 同步", "❌ 没有有效的 IP 结果，跳过自动同步")
            return
        ...
        _send_notification("GitHub 同步", f"✅ 已推送 {len(ip_lines)} 个 IP 到 {username}/{repo} ({branch})")
    except subprocess.CalledProcessError as e:
        err = e.stderr.decode() if e.stderr else str(e)
        _send_notification("GitHub 同步", f"❌ Git push 失败: {err[:200]}")
    except Exception as e:
        _send_notification("GitHub 同步", f"❌ Git push 异常: {str(e)[:200]}")
...
def _send_notification(title, content):
    ...
    if ewxp and cfg.get("WXPUSHER_APP_TOKEN"):
        try:
            payload = {"appToken": cfg["WXPUSHER_APP_TOKEN"], "content": content, "summary": title, "uids": cfg.get("WXPUSHER_UIDS", [])}
            requests.post(cfg.get("WXPUSHER_API_URL", "https://wxpusher.zjiecode.com/api/send/message"), json=payload, timeout=5)
        except:
            pass
    if epp and cfg.get("PUSHPLUS_TOKEN"):
...
def _run_collect_sync():
    ...
    if not live:
        _send_notification("合并历史检测同步", "⚠️ 所有历史 IP 均离线或没有历史记录")
        return

    result_lines = [f"📡 活着的IP: {len(live)} 个"]
    has_edgetunnel = False
    has_github = False
    edt_ok = 0
    edt_total = 0
    gh_ok = False
    ...
    if e_cfg.get("EDGETUNNEL_ENABLED"):
        ...
        http_eps = [e for e in endpoints if e.get("enabled") and e.get("type") != "kv"]
        has_kv_old = e_cfg.get("EDGETUNNEL_KV_MODE") and e_cfg.get("CF_ACCOUNT_ID") and e_cfg.get("KV_NAMESPACE_ID") and e_cfg.get("CF_API_TOKEN")
        edt_total = len(kv_eps) + len(http_eps) + (1 if has_kv_old else 0)
        for kv_ep in kv_eps:
            try:
                kv_tag = kv_ep.get("tag", "") or e_cfg.get("KV_TAG", "") or e_cfg.get("SYNC_TAG", "")
        ...
        for ep in http_eps:
            try:
                ...
                r = sess.post(f"{ep_url}/admin/ADD.txt", data="\n".join(ep_lines).encode('utf-8'),
                          headers={"Content-Type": "text/plain; charset=utf-8", "User-Agent": "CFST-Sync/1.0"}, timeout=15)
                if r.ok: edt_ok += 1
            except:
                pass
        result_lines.append(f"☁️ Edgetunnel: {edt_ok}/{edt_total}" if edt_total > 0 else "☁️ Edgetunnel: 无可用端点")
    ...
    if cfg.get("GITHUB_SYNC_ENABLED"):
        ...
        if token and username and repo:
            ...
            try:
                ...
                subprocess.run(["git", "-C", BASE_DIR, "push", "origin", branch], capture_output=True, timeout=180, check=True)
                gh_ok = True
                result_lines.append(f"🐙 GitHub: ✅ 推送 {len(live)} 个IP到 {username}/{repo} ({branch})")
            except subprocess.CalledProcessError as e:
                result_lines.append(f"🐙 GitHub: ❌ push 失败")
            except Exception as e:
                result_lines.append(f"🐙 GitHub: ❌ {str(e)[:80]}")
        else:
            result_lines.append("🐙 GitHub: ❌ 配置不完整")

    if not has_edgetunnel and not has_github:
        result_lines.append("⚠️ Edgetunnel 和 GitHub 均未启用，仅检测了 IP 连通性")

    content = "\n".join(result_lines)
    _send_notification("合并历史检测同步", content)
...
def api_edgetunnel_collect_sync():
    ...
    live = _collect_and_retest_ips()
    if not live:
        _send_notification("Edgetunnel 合并历史同步", "❌ 没有有效的历史记录或全部离线")
    ...
    if not kv_eps and not http_eps and not has_kv_old:
        _send_notification("Edgetunnel 合并历史同步", "❌ 没有可用的同步端点")
    ...
    success_count = sum(1 for r in results if "✅" in r)
    total = len(results)
    note = f"📡 合并历史检测同步到 Edgetunnel\n活着的IP: {len(live)} 个\n同步结果: {success_count}/{total} 成功"
    if success_count < total:
        fails = [r for r in results if "❌" in r][:3]
        note += "\n" + "\n".join(fails)
    _send_notification("Edgetunnel 合并历史同步", note)
...
def api_github_collect_sync():
    ...
    cfg = load_config()
    if not cfg.get("GITHUB_SYNC_ENABLED"):
        _send_notification("GitHub 合并历史同步", "❌ GitHub 同步未启用")
        return jsonify({"ok": False, "error": "GitHub 同步未启用"}), 400

    live = _collect_and_retest_ips()
    if not live:
        _send_notification("GitHub 合并历史同步", "❌ 没有有效的历史记录或全部离线")
        return jsonify({"ok": False, "error": "没有有效的历史记录或全部离线"}), 400
    ...
    if not token or not username or not repo:
        _send_notification("GitHub 合并历史同步", f"❌ 配置不完整\n活着的IP: {len(live)} 个未同步")
    ...
    for attempt in range(max_retries):
        try:
            ...
            subprocess.run(["git", "-C", BASE_DIR, "push", "origin", branch],
                         capture_output=True, timeout=process_timeout, check=True)
            _send_notification("GitHub 合并历史同步", f"✅ 已推送 {len(live)} 个IP到 {username}/{repo} ({branch})")
            return jsonify({"ok": True, "live_count": len(live), "details": [f"GitHub ✅ 推送 {len(live)} 个IP到 {username}/{repo} ({branch})"]})
        except subprocess.CalledProcessError as e:
            if attempt < max_retries - 1:
                time.sleep(retry_delay)
                continue
            err = e.stderr.decode() if e.stderr else str(e)[:100]
            _send_notification("GitHub 合并历史同步", f"❌ Git push 失败: {err}\n活着的IP: {len(live)} 个未同步")
            return jsonify({"ok": False, "error": f"Git push 失败: {err}", "live_count": len(live)}), 500
        except Exception as e:
            if attempt < max_retries - 1:
                time.sleep(retry_delay)
                continue
            _send_notification("GitHub 合并历史同步", f"❌ Git push 异常: {str(e)[:100]}\n活着的IP: {len(live)} 个未同步")
            return jsonify({"ok": False, "error": f"Git push 异常: {str(e)[:100]}", "live_count": len(live)}), 500

    _send_notification("GitHub 合并历史同步", f"❌ 超过重试次数\n活着的IP: {len(live)} 个未同步")
...
def api_github_sync_latest():
    ...
    cfg = load_config()
    if not cfg.get("GITHUB_SYNC_ENABLED"):
        _send_notification("GitHub 同步", "❌ GitHub 同步未启用，请在配置中开启")
        return jsonify({"ok": False, "error": "GitHub 同步未启用"}), 400
    ...
    if not row:
        ip_txt_path = os.path.join(BASE_DIR, cfg.get("OUTPUT_FILE", "ip.txt"))
        if not os.path.exists(ip_txt_path):
            _send_notification("GitHub 同步", "❌ 没有已完成的历史记录，无法同步")
    ...
    if not ip_lines:
        _send_notification("GitHub 同步", "❌ 没有有效的 IP 结果")
    ...
    if not token or not username or not repo:
        _send_notification("GitHub 同步", f"❌ 配置不完整，{len(ip_lines)} 个 IP 未同步")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                continue
            err = e.stderr.decode() if e.stderr else str(e)[:100]
            _send_notification("GitHub 同步", f"❌ Git push 失败: {err}")
            return jsonify({"ok": False, "error": f"Git push 失败: {err}"}), 500
        except Exception as e:
            if attempt < max_retries - 1:
                time.sleep(retry_delay)
                continue
            _send_notification("GitHub 同步", f"❌ Git push 异常: {str(e)[:100]}")
            return jsonify({"ok": False, "error": f"Git push 异常: {str(e)[:100]}"}), 500

    _send_notification("GitHub 同步", "❌ 超过重试次数")
